...
describe Text do
  ...
  it "should trigger two BANs when created" do
    Api.should_receive(:call_p).once.with("http://127.0.0.1", :ban, "/v[0-9]+/texts#{INVALIDATE_COLLECTION_DEFAULT.first}")
  ...
  it "should trigger three BANs when updated" do
    ...
    Api.should_receive(:call_p).once.with("http://127.0.0.1", :ban, "/v[0-9]+/texts/#{m.id}#{INVALIDATE_MEMBER_DEFAULT.first}")
    Api.should_receive(:call_p).once.with("http://127.0.0.1", :ban, "/v[0-9]+/texts#{INVALIDATE_COLLECTION_DEFAULT.first}")
    Api.should_receive(:call_p).once.with("http://127.0.0.1", :ban, "/v[0-9]+/dictionaries/app/foo_app/locale/sv-SE($|\\?)")
 	  m.save!
  end
